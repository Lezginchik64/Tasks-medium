...
# 3
print(max(mixed_list, key=lambda x: (isinstance(x, int), x)))   # Сначала сравнивается ТИП: True (число) всегда > False (не число)
                                                                # Только если типы одинаковые, сравниваются значения
